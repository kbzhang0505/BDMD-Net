    ...
    def __init__(
        ...
        for _ in range(num_blocks):
            self.blocks.append(nn.ModuleList([
                DSCB_MSA(dim=dim, dim_head=dim_head, heads=heads),
...
class DSCB_MSA(nn.Module):
    ...
    def forward(self, x_in, y_in, z_in):
        """
...
class low_light_transformer(nn.Module):
    def __init__(self, nf=64, nframes=5, groups=8, front_RBs=5, back_RBs=10, center=None,
        ...
        self.upT1 = Upsample2(int(nf * 4))
        self.upT2 = Upsample2(int(nf * 2))
        # self.upC1 = Upsample4(int(nf * 4))
        # self.upC2 = Upsample2(int(nf * 2))

        self.conv_last = nn.Conv2d(nf, 3, 3, 1, 1, bias=True)
        self.conv_T = nn.Conv2d(nf*3, nf, 3, 1, 1, bias=True)
        ...
        # self.transformer13 = IGAB(dim=nf, dim_head=nf, heads=1, num_blocks=4)
        # self.transformer14 = IGAB(dim=nf, dim_head=nf, heads=1, num_blocks=4)

        self.transformer20 = DSCB(dim=nf * 2, dim_head=nf * 2, heads=1, num_blocks=4)
        ...
        # self.transformer23 = IGAB(dim=nf * 2, dim_head=nf * 2, heads=1, num_blocks=4)
        # self.transformer24 = IGAB(dim=nf * 2, dim_head=nf * 2, heads=1, num_blocks=4)
        self.transformer30 = DSCB(dim=nf * 4, dim_head=nf * 2, heads=2, num_blocks=4)
        ...
        self.fusion_layer = AdaptiveWeightFusionFreqDomain(nf*2)
    #
    # def visualize_feature(self, feature_map, num_channels=5):
    #     # 假设 feature_map 形状是 (1, 128, 64, 64)
    #     feature_map = feature_map.squeeze().detach().cpu().numpy()  # 变为 (128, 64, 64)
    #
    #     # 选择激活值最高的几个通道
    #     channel_sums = feature_map.sum(axis=(1, 2))  # 对每个通道的像素求和
    #     top_channels = channel_sums.argsort()[-num_channels:]  # 选择最高的通道索引
    #
    #     # 将这些通道可视化
    #     for i, channel in enumerate(top_channels):
    #         plt.subplot(1, num_channels, i + 1)
    #         plt.imshow(feature_map[channel], cmap='hot')
    #         plt.axis('off')
    #
    #     plt.show()
    ...
    def forward(self, x, x2, x3):
        ...
        L_T2 = self.transformer20(L1_fea_x2, L1_fea_x2, L1_fea_x2)
        L_T2 = self.transformer20(L_T2, L1_fea_2, L1_fea_2)
        ...
        L2_E = self.transformer21(L_T2, L_T2, L_T2, illu_fea2)
        L2_E = self.transformer21(L2_E, L2_E, L2_E, illu_fea2)

        L_T3 = self.transformer30(L1_fea_x3, L1_fea_x3, L1_fea_x3)
        L_T3 = self.transformer30(L_T3, L1_fea_3, L1_fea_3)
        ...
        L3_E = self.transformer31(L_T3, L_T3, L_T3, illu_fea3)
        L3_E = self.transformer31(L3_E, L3_E, L3_E, illu_fea3)
        # q2 = self.down_2(L1_T_2) + L1_T_3
        q2 = torch.cat((self.down_2(L2_E), L3_E), dim=1)
        q2 = self.q2conv(q2)
        L3_E = self.transformer31(q2, L3_E, L3_E, illu_fea3)
        L3_E = self.transformer31(L3_E, L3_E, L3_E, illu_fea3) + self.down_2(L2_E)
        ...
        L_T = self.fusion_layer(L2_D, L3_D)
        L_T = self.upT2(L_T)

        # fea_C_last = self.conv_last_C(fea)
...
class AdaptiveWeightFusionFreqDomain(nn.Module):
    ...
    def forward(self, x1, x2):
        ...
        concat_mag = torch.cat([mag_u, mag_v], dim=1)
        weights_mag = self.reweight_magnitude(concat_mag).view(B, 2, C).permute(1, 0, 2).softmax(dim=0)
        weights_mag = weights_mag.unsqueeze(-1).unsqueeze(-1)

        concat_pha = torch.cat([pha_u, pha_v], dim=1)
        weights_pha = self.reweight_phase(concat_pha).view(B, 2, C).permute(1, 0, 2).softmax(dim=0)
        weights_pha = weights_pha.unsqueeze(-1).unsqueeze(-1)

        # Apply weights
        fused_mag = mag1 * weights_mag[0] + mag2 * weights_mag[1]
        fused_pha = pha1 * weights_pha[0] + pha2 * weights_pha[1]
